...
import { Card, Typography, Descriptions, Tag, Button, Row, Col, Spin, message, Progress } from 'antd'
import { CheckCircleOutlined, CloseCircleOutlined, ReloadOutlined, QuestionCircleOutlined } from '@ant-design/icons'
...
function StatusIcon({ status }) {
  ...
  if (status === false || status === 0 || status === 'DOWN' || status === 'unhealthy') return <CloseCircleOutlined style={{ color: '#f5222d', fontSize: 18 }} />
...
function StatusTag({ status }) {
  ...
  return <Tag color={healthy ? 'green' : 'red'} icon={healthy ? <CheckCircleOutlined /> : <CloseCircleOutlined />}>{label}</Tag>
}
...
export default function HealthCheck() {
  ...
  const healthOk = health ? isHealthy(health.status) : false
              ...
              {readiness ? (
                <div>
                  ...
                  <Descriptions bordered column={1} size="small" style={{ marginTop: 16 }}>
                    <Descriptions.Item label="状态">
                      <StatusTag status={readiness.status} />
                    </Descriptions.Item>
